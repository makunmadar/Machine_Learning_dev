...
combo_samples = np.load("Samples_combo_orig.npy")
...
combo_thresholds = np.load("Thresholds_combo_orig.npy")
# Load the criterion data
combo_criterion = np.load("Criterion_combo_orig.npy")
combo_criterion = np.ma.masked_invalid(combo_criterion)
# combo_criterion = combo_criterion

# Plotting the likelihood ratio distribution with the threshold distribution
plt.hist(np.log10(combo_criterion), bins=100, label='Likelihood ratio')
plt.hist(np.log10(combo_thresholds), bins=100, label='Acceptance threshold', histtype='step')
...
combo_predictions = np.load("Predictions_combo_orig.npy")
...
combo_predictions_raw = np.load("Predictions_combo_orig_raw.npy")
...
axs[0, 0].legend()
axs[0, 1].legend()
axs[2, 0].set_ylabel('Log$_{10}$(dN(>S)/dz) [deg$^{-2}$]', fontsize=16)
axs[2, 1].set_ylabel(r"Log$_{10}$(LF (Mpc/h)$^{-3}$ (mag$_{AB}$)$^{-1}$)", fontsize=16)

axs[-1, 0].set_xlabel('Redshift', fontsize=16)
axs[-1, 1].set_xlabel(r"M$_{AB}$ - 5log(h)", fontsize=16)
...
colorbar.set_label('Iteration', fontsize=16)
