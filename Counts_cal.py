...
plt.axvline(x=np.log10(f_e), linestyle='dashed', c='tab:gray', alpha=0.7, label='WFIRST flux limit')
plt.xlim([0.2, 1.0])
plt.ylim([1, 4.0])
plt.tight_layout()
#plt.savefig("Plots/100bestMCMC_counts_WFIRST.pdf")
...
plt.ylim([1, 4.0])
plt.tight_layout()
plt.text(0.7, 3.5, '1.0$\less$z$\less$2.0', c='tab:blue')
# plt.text(0.8, 3.7, '0.9$\less$z$\less$1.6', c='black')
#plt.savefig("Plots/50bestMCMC_counts_WFIRST.pdf")
plt.show()

# We will use interpolation to find the number counts corresponding to f > 1x10^-16 erg-1s-1cm-2
...
h24counts_path = 'Data/Data_for_ML/MCMC/counts_best_H24/'
h24counts_filenames = os.listdir(h24counts_path)
h24counts_filenames.sort(key=lambda f: int(re.sub('\D', '', f)))

h24list_counts, h24counts_bins = counts_generation(h24counts_filenames, h24counts_path)
C_i = [j for j, e in enumerate(h24list_counts[0]) if e != 0]
plt.plot(h24counts_bins[C_i], h24list_counts[0][C_i])
plt.plot(counts_bins[C_i], list_counts[61][C_i], '-', c='tab:red')
...
C_i = [j for j, e in enumerate(h24list_counts[0]) if e != 0]
interp_func = interp1d(h24counts_bins[C_i], h24list_counts[0][C_i], kind='linear', fill_value='extrapolate')
targed_x = np.log10(f_e)
target_y = interp_func(targed_x)
print("\n")
print("Number count with H=24 limit at Euclid limit for best model: ", 10**target_y, " deg^-2")