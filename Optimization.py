...
def mcmc_updater(curr_state, curr_likeli, model, obs_x, obs_y, sigma, mae_weighting,
    ...
    prop_likeli, prop_pred = likelihood(proposal_state, model, obs_x, obs_y, sigma,
    ...
    if accept_crit > accept_threshold:
        ...
        return proposal_state, prop_likeli, prop_pred, accept_crit, accept_threshold

    else:
        # print("Proposed state rejected")
        return curr_state, curr_likeli, curr_pred, accept_crit, accept_threshold
...
def metropolis_hastings(
    ...
    for i in range(num_samples):
        ...
        curr_state, curr_likeli, curr_pred, accept_crit, thresh = mcmc_updater(
    ...
    return samples, predictions, likeli, criterion, threshold
...
def likelihood(params, models, obs_x, obs_y, sigma, mae_weighting, pred_bins):
    ...
    predictions_mean = predictions_mean[0]

    predictions = 10 ** predictions_mean
    ...
    abs_diff = np.abs(pred - obs_y)/sigma
    ...
    likelihood = np.exp(-weighted_mae / 0.05)

    return likelihood, predictions_mean
...
for n in range(n_walkers):
    ...
    samples, predictions, likelihoods, criterion, threshold = metropolis_hastings(
        model=members,
        obs_x=obs_x,
        obs_y=obs_y,
        sigma=sigma,
...
flattened_likelihoods = (1/(2*0.05)) * flattened_likelihoods
flattened_criterion = np.reshape(n_criterion, (-1, 1))
flattened_threshold = np.reshape(n_threshold, (-1, 1))
...
np.save('Samples_combo_orig.npy', flattened_samples)
np.save('Likelihoods_combo_orig.npy', flattened_likelihoods)
np.save('Predictions_combo_orig.npy', flattened_predictions)
np.save('Criterion_combo_orig.npy', flattened_criterion)
np.save('Thresholds_combo_orig.npy', flattened_threshold)
np.save('Predictions_combo_orig_raw.npy', n_predictions)
